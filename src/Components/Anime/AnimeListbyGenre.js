...
import { GET_ANIME_LIST_GENRE } from '../../actions/anime';
import { AnimeListContext } from '../../context/Context';
// Spy-x-Family-Anya-Forger

function AnimeListbyGenre(){
    const navigate = useNavigate();
    const [numberPage, setNumberPage] = useState([]);
    const [page, setPage] = useState(1);
    const { dataContext, setDataContext } = useContext(AnimeListContext);
    const [getListAnimeGenre, { data: animeListbyGenre }] = useLazyQuery(GET_ANIME_LIST_GENRE);
    // const { data: animeListbyGenre, error: GetAnimeListbyGenreError, loading: isAnimeListbyGenreLoading } = useQuery(GET_ANIME_LIST_GENRE);
    // const [getListAnimeTrend, { data }] = useLazyQuery(GET_ALL_TREND_ANIME);
    React.useEffect(() => {
        if(dataContext && typeof dataContext.filter?.genre !== 'undefined'){
            getListAnimeGenre({
                variables:{
                    page: page,
                    genre: dataContext.filter.genre
                }
            });
        }
            ...
            >
                ...
                { !animeListbyGenre ?
                    <div
                        ...
                        `}
                    > Loading... </div>
                        : animeListbyGenre.Page.media.map((row, key)=>(
                        <li
                            className={css`  
                                float: left;
                                margin: 10px;
            ...
            <div>
                ...
                >
                    ...
                    <button
                        ...
                        `}
                        // class="last"
                        onClick={()=>handleChangePage(animeListbyGenre?.Page.pageInfo.lastPage, 'last')}
